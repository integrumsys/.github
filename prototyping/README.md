...
A corner treatment must not shorten a letter. M was drawn as a single
polyline, which made its two peaks interior corners; the chamfer then cut them
back and M sat 10.9% (B) and 8.4% (C) of cap height below every other letter.
Its stems are now separate strokes, as N's already were, so the treatment
applies only to the middle vertex. All eight letters now measure flush.

Then `out/A-square/`, `B-chamfer/` and `C-hex/` hold that direction's ten SVGs under identical filenames, so any two directions can be
opened side by side.
...
D (a lighter, wider-tracked cut of A) was explored and cut.
...
- **The mark's frame is light for a lockup.** At cap height its 47-unit frame
  ...
  `mark_height()` targets a frame at 0.8x the stroke. The alternative is
  thickening the mark's frame, which would change the locked artwork.
